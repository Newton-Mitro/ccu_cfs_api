...
import { JointSTDAccountOpeningPolicyValidator } from '../policy/open/joint-std-account-opening-policy-validator';
import { OrganizationalSTDAccountOpeningPolicyValidator } from '../policy/open/organizational-std-account-opening-policy-validator';
import { PersonalSTDAccountOpeningPolicyValidator } from '../policy/open/personal-std-account-opening-policy-validator';
import { STDAccountCommonOpeningPolicyValidator } from '../policy/open/std-account-common-opening-policy-validator';

export class OpenLikeShortTimeDepositAccount
  implements IOpenableSubsidiaryLedger
{
  OpenSubsidiaryLedgerAccount(
    ...
    policyValidator = new STDAccountCommonOpeningPolicyValidator();
    ...
    if (accountType === AccountType.Personal) {
      policyValidator = new PersonalSTDAccountOpeningPolicyValidator();
      policyValidator.ValidatePolicy(
        accountType,
        controlLedger,
        accountName,
    ...
    if (accountType === AccountType.Joint) {
      policyValidator = new JointSTDAccountOpeningPolicyValidator();
      policyValidator.ValidatePolicy(
        accountType,
        controlLedger,
        accountName,
    ...
    if (accountType === AccountType.Organizational) {
      policyValidator = new OrganizationalSTDAccountOpeningPolicyValidator();
      policyValidator.ValidatePolicy(
        accountType,
        controlLedger,
        accountName,
    ...
    accountModel.setAccountNumber('STD-' + UUID.makeAccountId(6)); // How to generate Account Number???
